...
function getAllPossibleCoordsQueen(pieceCoord) {
    ...
    var resBishup = getAllPossibleCoordsBishop(pieceCoord)
    var res = resRook.concat(resBishup)
    return res
}
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    for(var i = pieceCoord.i-2 ; i <= pieceCoord.i+2 ; i++){ 
        for(var j = pieceCoord.j -2 ; j <= pieceCoord.j+2 ; j++){ 
            ...
            if (!isEmptyCell(coord)) continue;
            if (i === pieceCoord.i && j !== pieceCoord.j) continue
            else if(i !== pieceCoord.i && j === pieceCoord.j)continue
            if(
                Math.abs((pieceCoord.i-coord.i) + (pieceCoord.j - coord.j)) === 1 ||
                Math.abs((pieceCoord.i-coord.i) + (pieceCoord.j - coord.j)) === 3 
            )
                res.push(coord);
        }
    } 
